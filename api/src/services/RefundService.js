 ...
 */
async function createRefundRequest(request) {
 ...
 */
async function getPendingRefunds(limit = MAX_REFUNDS_PER_RUN) {
 ...
 */
async function executeRefundTransfer(toAddress, amountCents) {
    ...
    try {
        ...
        const chain = isProduction ? chains_1.base : chains_1.baseSepolia;
 ...
 */
async function processSingleRefund(refund) {
    ...
    try {
        const result = await executeRefundTransfer(refund.payer_address, refund.amount_cents);
        if (result.success) {
 ...
 */
async function getRefundStatus(clipVoteId) {

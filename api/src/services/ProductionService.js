...
class ProductionService {
     ...
     */
    async generatePoster(studioId, request) {
        const production = await this.getProduction(request.productionId);
        if (production.studioId !== studioId) {
            throw new ForbiddenError('You do not own this production');
        }
        ...
        if (mainPoster) {
            ...
            await queryOne('UPDATE productions SET poster_url = $1, updated_at = NOW() WHERE id = $2', [mainPoster.cdnUrl || mainPoster.url, request.productionId]);
        }
        return {
            poster: mainPoster,
            concepts,
     ...
     */
    async generatePosterPrompt(productionId) {
        const production = await this.getProduction(productionId);
        return this.gradient.generatePosterPrompt(production.title, production.logline, production.genre);
    }
    // ---------------------------------------------------------------------------
    // Collaborators
    // ---------------------------------------------------------------------------

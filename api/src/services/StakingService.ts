...
export async function getOrCreateDefaultPool() {
  ...
  if (!pool) {
    ...
    pool = await prisma.stakingPool.create({
      data: {
        name: config.staking.defaultPoolName,
        description: 'Default staking pool for all agents. Stake your earnings and earn rewards.',
        min_stake_amount_cents: BigInt(config.staking.minStakeAmountCents),
        min_stake_duration_seconds: config.staking.minStakeDurationSeconds,
        apy_basis_points: config.staking.defaultApyBasisPoints,
        is_default: true,
        is_active: true
      }
    });
...
export async function unstake(params: UnstakeParams) {
  ...
  if (now < existingStake.can_unstake_at) {
    const remainingSeconds = Math.ceil(
      (existingStake.can_unstake_at.getTime() - now.getTime()) / 1000
    );
    throw new Error(
      `Cannot unstake yet. Minimum stake duration not met. Wait ${remainingSeconds} more seconds.`
    );
  }
 ...
 */
export async function calculateRewards(stakeId: string) {
  ...
  if (secondsElapsed < config.staking.rewardCalculationIntervalSeconds) {
    return;
  }
  ...
  const rewardCents = Math.floor(
    (stakeAmount * apyBasisPoints * secondsElapsed) / (10000 * SECONDS_IN_YEAR)
  );
 ...
 */
export async function calculateAllRewards() {
 ...
 */
export async function getStakingStatus(agentId: string): Promise<StakingStatus> {
  ...
  const totalEarnedCents = stakes.reduce(
    (sum, s) => sum + Number(s.earned_rewards_cents),
    0
  );

  const claimedRewardsCents = stakes.reduce(
    (sum, s) => sum + Number(s.claimed_rewards_cents),
    0
  );
 ...
 */
export async function getStakingEarnings(agentId: string): Promise<StakingEarnings> {
  ...
  const totalEarnedCents = rewards.reduce(

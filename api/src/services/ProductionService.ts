...
import type { StoredAsset } from '../types/spaces';
...
function validateGenre(genre: string): genre is ProductionGenre {
  const validGenres: ProductionGenre[] = [
    'action', 'comedy', 'drama', 'horror', 'sci_fi', 'fantasy',
    'documentary', 'animation', 'thriller', 'romance', 'western', 'noir', 'experimental'
  ];
  return validGenres.includes(genre as ProductionGenre);
}

function validateAspectRatio(ratio: string): ratio is AspectRatio {
  const validRatios: AspectRatio[] = ['16:9', '9:16', '1:1', '4:3', '2.35:1'];
...
export class ProductionService {
  private gradient: GradientClient;
  private spaces: SpacesClient;
  ...
  async createProduction(
    ...
    const existing = await queryOne(
      'SELECT id FROM productions WHERE slug = $1',
      [slug]
    );
    ...
    const production = await queryOne(
      ...
      [
        ...
        'development' as ProductionStatus,
      ]
    );
   ...
   */
  async getProductionBySlug(slug: string): Promise<Production> {
    const production = await queryOne(
      `SELECT p.*, a.name as studio_name, a.display_name as studio_display_name
       FROM productions p
       JOIN agents a ON p.studio_id = a.id
  ...
  async listStudioProductions(
    ...
    const countResult = await queryOne(
      'SELECT COUNT(*) as count FROM productions WHERE studio_id = $1',
      [studioId]
    );

    return {
      productions: productions.map((p: Record<string, unknown>) => this.mapProductionRow(p)),
  ...
  async updateProductionStatus(
    ...
    const updated = await queryOne(
      `UPDATE productions SET status = $1, updated_at = NOW()
  ...
  async createShot(
    ...
    if (sequenceIndex === undefined) {
      const maxSeq = await queryOne(
        'SELECT COALESCE(MAX(sequence_index), -1) as max_seq FROM shots WHERE production_id = $1',
    ...
    await queryOne(
      'UPDATE productions SET shot_count = shot_count + 1, updated_at = NOW() WHERE id = $1',
      [request.productionId]
    );
   ...
   */
  async getShotManifest(productionId: string): Promise<ShotManifest> {
    const production = await this.getProduction(productionId);

    const shots = await queryAll(
      `SELECT * FROM shots 
    ...
    const mappedShots = shots.map((s: Record<string, unknown>) => this.mapShotRow(s));
    const completedShots = mappedShots.filter((s: Shot) => s.status === 'approved').length;
    const pendingShots = mappedShots.filter((s: Shot) => 
      s.status === 'draft' || s.status === 'queued' || s.status === 'generating' || s.status === 'review'
    ).length;
    const totalDuration = mappedShots.reduce((sum: number, s: Shot) => sum + s.durationSec, 0);
  ...
  async updateShotStatus(
    studioId: string,
    request: UpdateShotStatusRequest
  ): Promise<UpdateShotStatusResponse> {
    ...
    const params: (string | number | null)[] = [request.status];
    ...
    const updated = await queryOne(
      `UPDATE shots SET ${updates.join(', ')} WHERE id = $${params.length} RETURNING *`,
      params
    );
    ...
    if (request.status === 'approved') {
      await queryOne(
        'UPDATE productions SET completed_shot_count = completed_shot_count + 1, updated_at = NOW() WHERE id = $1',
        [shot.productionId]
      );
    }
  ...
  async generateShot(
    ...
    await queryOne(
      'UPDATE shots SET status = $1, updated_at = NOW() WHERE id = $2',
      ['generating', request.shotId]
    );
    ...
    try {
      ...
      const result = await this.gradient.generateShot(refinedPrompt, {
        ...
        aspectRatio: shot.aspectRatio as '16:9' | '9:16' | '1:1',
      ...
      await queryOne(
        `UPDATE shots SET 
          ...
          updated_at = NOW()
        WHERE id = $3`,
        [result.id, 'generating', request.shotId]
      );
   ...
   */
  async pollShotGeneration(shotId: string): Promise<Shot> {
    ...
    if (status.status === 'completed' && status.video_url) {
      ...
      const asset = await this.spaces.uploadFromUrl(
        status.video_url,
        `productions/${shot.productionId}/shots/${shot.id}/v${shot.version}.mp4`,
        {
      ...
      await queryOne(
        `UPDATE shots SET 
          ...
          updated_at = NOW()
        WHERE id = $4`,
        [asset.url, status.thumbnail_url || null, 'review', shot.id]
      );
      ...
      await queryOne(
        `UPDATE shots SET 
          ...
          updated_at = NOW()
        WHERE id = $3`,
        ['rejected', status.error || 'Unknown error', shot.id]
      );
    }
  ...
  async addCollaborator(
    ...
    await queryOne(
      `INSERT INTO production_collaborators (production_id, agent_id, role)
       VALUES ($1, $2, $3)
       ON CONFLICT (production_id, agent_id) DO UPDATE SET role = $3`,
      [productionId, agentId, role]
    );
  ...
  async removeCollaborator(
    ...
    await queryOne(
      'DELETE FROM production_collaborators WHERE production_id = $1 AND agent_id = $2',
      [productionId, agentId]
    );
  }
  ...
  private async getShot(shotId: string): Promise<Shot> {
    const shot = await queryOne(
  ...
  private mapProductionRow(row: Record<string, unknown>): Production {
    return {
      id: row.id as string,
      ...
      genre: row.genre as ProductionGenre,
      tags: JSON.parse(row.tags as string || '[]'),
      rating: row.rating as Production['rating'],
      status: row.status as ProductionStatus,
      currentPhase: row.current_phase as string || '',
      createdAt: new Date(row.created_at as string),
      updatedAt: new Date(row.updated_at as string),
      releaseDate: row.release_date ? new Date(row.release_date as string) : undefined,
  ...
  private mapShotRow(row: Record<string, unknown>): Shot {
    return {
      ...
      sequenceIndex: parseInt(row.sequence_index as string, 10),
      promptText: row.prompt_text as string,
      negativePrompt: row.negative_prompt as string | undefined,
      aspectRatio: row.aspect_ratio as AspectRatio,
      durationSec: parseInt(row.duration_sec as string || '5', 10),
      cameraMotion: row.camera_motion as Shot['cameraMotion'],
      ...
      version: parseInt(row.version as string || '1', 10),
      createdAt: new Date(row.created_at as string),
      updatedAt: new Date(row.updated_at as string),
...
let serviceInstance: ProductionService | null = null;

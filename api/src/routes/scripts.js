 ...
 */
router.get('/:scriptId', requireAuth, asyncHandler(async (req, res) => {
 ...
 */
router.patch('/:scriptId', requireAuth, asyncHandler(async (req, res) => {
    const { scriptId } = req.params;
 ...
 */
router.post('/:scriptId/submit', requireAuth, asyncHandler(async (req, res) => {
    const { scriptId } = req.params;
    const script = await prisma.script.findUnique({
        where: { id: scriptId },
    ...
    const rateError = canSubmitScript(script.studio.script_count, script.studio.last_script_at);
    if (rateError) {
 ...
 */
router.delete('/:scriptId', requireAuth, asyncHandler(async (req, res) => {
    const { scriptId } = req.params;
    const script = await prisma.script.findUnique({
        where: { id: scriptId },
        include: { studio: true },

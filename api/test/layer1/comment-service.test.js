...
describe('Layer 1 - Comment Service', () => {
  ...
  afterAll(async () => {
    try {
      ...
      if (postId) await db.query('DELETE FROM posts WHERE id = $1', [postId]);
  ...
  it('sets up agent, submolt, and post for comments', async () => {
    ...
    submoltName = `comsub_${Date.now().toString(36)}`;
    const sub = await request(app)
      .post('/api/v1/submolts')
      .set('Authorization', `Bearer ${agentKey}`)
      .send({ name: submoltName, display_name: 'Comment Submolt', description: 'Testing' });
    ...
    const post = await request(app)
      .post('/api/v1/posts')
      .set('Authorization', `Bearer ${agentKey}`)
      .send({ submolt: submoltName, title: 'Post for Comments', content: 'Discuss here' });
  ...
  it('retrieves comments for post', async () => {
    const res = await request(app)
  ...
  it('returns 404 for comment on non-existent post', async () => {
    const res = await request(app)
